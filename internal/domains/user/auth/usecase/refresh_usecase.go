...
func (u *authUsecase) Refresh(ctx context.Context, req dto.RefreshTokenRequest) (*dto.LoginResponse, error) {
	...
	if refreshToken == "" {
		return nil, apperrors.New("VALIDATION_ERROR", "Refresh token wajib diisi", http.StatusBadRequest)
	}

	claims, err := commonauth.ParseRefreshToken(refreshToken, u.cfg.JWTSecret)
	if err != nil {
		return nil, apperrors.New("UNAUTHORIZED", "Refresh token tidak valid atau expired", http.StatusUnauthorized)
	}

	user, err := u.repo.FindByID(ctx, claims.Subject)
	if err != nil {
		return nil, apperrors.New("INTERNAL_ERROR", "Gagal memproses refresh token", http.StatusInternalServerError)
	}
